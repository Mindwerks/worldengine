...
class BiomeSimulation(object):
    ...
    def execute(world, seed):
        ...
        w.biome = biome
        l=[]
        used_keys=set(biome_cm.keys())
        unused_keys=set(biome_colors.keys())
        d=used_keys.difference(unused_keys)
        #print("key,d",d)
        d2=unused_keys.difference(used_keys)
        #print("key,d2",d2)
        if d!=set() or d2!= set():
            print("key set difference:\n",d,d2)
        else:
            print("all keys used")
        #print("biome_cm")
        #for key in biome_cm:
        #    l.append((key,biome_cm[key]))
        #l.sort()
        #for i in l:
