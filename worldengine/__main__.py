...
from common import array_to_matrix, set_verbose, print_verbose
from draw import draw_ancientmap_on_file, draw_biome_on_file, \
    draw_ocean_on_file,  draw_precipitation_on_file, \
    draw_grayscale_heightmap_on_file, draw_simple_elevation_on_file, \
...
def __get_tag__(filename):
    with open(filename, 'rb') as ifile:
        # drop first byte, it should tell us the protobuf
        # version and it should be normally equal to 8
        data = ifile.read(1)
        if not data:
            return None
        done = False
    ...
    if pb and pi:
        ...
        try:
            ...
            try:
                ...
                raise Exception("Unable to load the world file neither as "
                                "protobuf or pickle file: %s" % str(e))
        ...
        raise Exception("The given worldfile does not seem a pickle or a "
                        "protobuf file.")
...
def main():
    ...
    if operation == 'world':
        ...
        world = generate_world(
            world_name, args.width, args.height, seed, args.number_of_plates,
            args.output_dir, step, args.ocean_level, world_format,
